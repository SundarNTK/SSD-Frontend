...
export default function Topbar({ onMenuClick }: TopbarProps) {
   ...
   */
  return (
    <header className="relative z-20 flex h-16 shrink-0 items-center justify-between gap-3 border-b border-gold-500/15 bg-navy-900 px-4 shadow-[0_8px_24px_-6px_rgba(0,0,0,0.14)] print:hidden sm:px-6">
          ...
          <AnimatePresence>
            {menuOpen && (
                <motion.div
                  ...
                  className="absolute right-0 top-[calc(100%+8px)] z-20 w-52 overflow-hidden rounded-xl border border-gold-500/20 bg-navy-900 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.6)]"
                >
                  {/* Repeated here because the trigger only shows the name at
                      lg and up — the clock takes that room back below it. */}
                  <div className="border-b border-gold-500/10 px-4 py-3 lg:hidden">
                  ...
                  <button
                    ...
                    className="flex w-full items-center gap-2.5 px-4 py-3 text-left text-[13px] text-ink-300 transition-colors hover:bg-crimson-500/10 hover:text-crimson-500"
                  >
                    <LogoutIcon /> Sign out
                  </button>
                </motion.div>
